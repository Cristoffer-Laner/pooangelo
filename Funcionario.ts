...
export class Funcionario {
    ...
    constructor(
      ...
      this.cargo = cargo;
      this.energia = Util.randomizar(30, 100);
    ...
    trabalhar(numeroDeHoras: number): void {
      this.energia -= Util.randomizar(1, 8) * numeroDeHoras;
      this.credibilidade += Util.randomizar(1, 2) * numeroDeHoras;
      this.validaEnergia();
    ...
    daleUmCafezin(): number {
      ...
      this.credibilidade -= Util.randomizar(2, 12);
      this.validaEnergia();
      this.validaCredibilidade();
    ...
    solicitarAumento(): boolean {
      const dado = Util.randomizar(0, 100);
